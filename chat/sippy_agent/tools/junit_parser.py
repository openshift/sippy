...
class JUnitParserTool(SippyBaseTool):
    ...
    def _extract_underlying_job_links(self, xml_content: str) -> List[Dict[str, str]]:
        ...
        for job_id in all_job_ids:
            underlying_jobs.append({"job_id": job_id, "url": f"https://prow.ci.openshift.org/view/gs/test-platform-results/logs/{job_id}"})
        ...
        for status, url in links:
            if "prow.ci.openshift.org" in url or url.startswith("http"):
                ...
                if job_id_match:
                    ...
                    underlying_jobs.append({"job_id": job_id, "url": url, "status": status.upper()})
    ...
    def _format_test_results(
        self, results: List[Dict[str, Any]], specific_test: Optional[str] = None, underlying_jobs: Optional[List[Dict[str, str]]] = None
    ) -> str:
    ...
    def _format_test_results_with_limit(
        self, results: List[Dict[str, Any]], underlying_jobs: Optional[List[Dict[str, str]]] = None, max_size_kb: int = 150
    ) -> tuple[str, int, int]:
