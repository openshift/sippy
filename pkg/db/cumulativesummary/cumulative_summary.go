...
func Backfill(dbc *db.DB, startDate, endDate civil.Date) error {
	...
	return backfillDateRange(store, startDate, endDate)
}

func backfillDateRange(store summaryStore, startDate, endDate civil.Date) error {
	...
	}).Info("backfilling cumulative summaries")

	for date := startDate; !date.After(endDate); date = date.AddDays(1) {
		updateStart := time.Now()
		for _, release := range releases {
			if err := store.UpdateDateForRelease(date, release); err != nil {
				return fmt.Errorf("updating cumulative summaries for %s release %s: %w", date, release, err)
			}
		}
		log.WithFields(log.Fields{
			"date":    date,
			"elapsed": time.Since(updateStart),
		}).Info("backfilled cumulative summaries for date")
	}

	log.WithField("elapsed", time.Since(loadStart)).Info("cumulative summary backfill complete")
	return nil
}
