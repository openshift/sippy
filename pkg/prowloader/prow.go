...
func (pl *ProwLoader) prowJobToJobRun(pj prow.ProwJob, release string, newJobRunsCtr *int, currentIndex, totalJobsToScan int) error {
	...
	if !foundProwJob {
		...
		dbProwJob = &models.ProwJob{
			...
			Release:     release,
			Variants:    pl.variantManager.IdentifyVariants(pj.Spec.Job, release, models.JobType{}),
		...
		saveDB := false
		newVariants := pl.variantManager.IdentifyVariants(pj.Spec.Job, release, models.JobType{})
