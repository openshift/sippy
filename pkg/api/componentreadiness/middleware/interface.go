...
type Middleware interface {
	...
	PreAnalysis(testKey crtest.ReportTestIdentification, testStats *crtype.ReportTestStats) error
	...
	PostAnalysis(testKey crtest.ReportTestIdentification, testStats *crtype.ReportTestStats) error

	// PreTestDetailsAnalysis gives middleware the opportunity to adjust inputs to the report status
	// prior to analysis.
	PreTestDetailsAnalysis(testKey crtest.TestWithVariantsKey, status *crtype.TestJobRunStatuses) error
}
