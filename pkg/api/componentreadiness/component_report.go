...
func GetReleaseDatesFromBigQuery(client *bqcachedclient.Client) ([]crtype.Release, []error) {
	generator := componentReportGenerator{
		client: client,
	}

	return api.GetDataFromCacheOrGenerate[[]crtype.Release](client.Cache, cache.RequestOptions{}, api.GetPrefixedCacheKey("CRReleaseDates~", generator), generator.GenerateReleaseDates, []crtype.Release{})
}
...
func (c *componentReportGenerator) GenerateReleaseDates() ([]crtype.Release, []error) {
	releases, err := api.GetReleasesFromBigQuery(c.client)
	if err != nil {
	...
	for _, release := range releases {
		...
		if release.GADate != nil {
			prior := util.Prior30Days(release.GADate)
			crRelease.GA30DayPrior = &prior
			crRelease.GADate = release.GADate
		}
...
func (c *componentReportGenerator) getFallbackBaseQueryStatus(allJobVariants crtype.JobVariants, release string, start, end time.Time) []error {
	...
	cachedFallbackTestStatuses, errs := api.GetDataFromCacheOrGenerate[*crtype.FallbackReleases](c.client.Cache, generator.cacheOption, api.GetPrefixedCacheKey("FallbackReleases~", generator), generator.getTestFallbackReleases, &crtype.FallbackReleases{})
...
func (f *fallbackTestQueryReleasesGenerator) getTestFallbackReleases() (*crtype.FallbackReleases, []error) {
	wg := sync.WaitGroup{}
	fallbackRelease := f.BaseRelease
	f.CachedFallbackTestStatuses = newFallbackReleases()

	releases, errs := GetReleaseDatesFromBigQuery(f.client)
	...
	for i := 0; i < 4; i++ {
		...
		go func(queryRelease *crtype.Release, queryStart, queryEnd time.Time) {
			defer wg.Done()
			// these could be new generator types if we wanted
			// to reuse individual fallback queries
			// like 4.17 and 4.16 base queries would share 4.15 && 4.14
			stats, errs := f.getTestFallbackRelease(queryRelease.Release, queryStart, queryEnd)
...
func (f *fallbackTestQueryReleasesGenerator) getTestFallbackRelease(release string, start, end time.Time) (crtype.ReportTestStatus, []error) {
	...
	testStatuses, errs := api.GetDataFromCacheOrGenerate[crtype.ReportTestStatus](f.client.Cache, generator.cacheOption, api.GetPrefixedCacheKey("FallbackBaseTestStatus~", generator), generator.getTestFallbackRelease, crtype.ReportTestStatus{})
...
func (f *fallbackTestQueryGenerator) getTestFallbackRelease() (crtype.ReportTestStatus, []error) {
	return f.getFallbackBaseQueryStatus(f.BaseRelease, f.BaseStart, f.BaseEnd)
}

func (f *fallbackTestQueryGenerator) getFallbackBaseQueryStatus(release string, start, end time.Time) (crtype.ReportTestStatus, []error) {
	before := time.Now()
...
func (c *componentReportGenerator) getTestStatusFromBigQuery() (crtype.ReportTestStatus, []error) {
	...
	if !c.IgnoreFallback {
		...
		go func() {
			defer wg.Done()
			c.getFallbackBaseQueryStatus(allJobVariants, c.BaseRelease.Release, c.BaseRelease.Start, c.BaseRelease.End)
		}()
	}
...
func (c *componentReportGenerator) matchBestBaseStats(testID crtype.ReportTestIdentification, testIdentification, baseRelease string, baseStats, sampleStats crtype.TestStatus, requiredConfidence int, approvedRegression *regressionallowances.IntentionalRegression, numberOfIgnoredSampleJobRuns int) (crtype.TestStatus, string, crtype.ReportTestStats) {
	...
	baseTestStats := c.assessComponentStatus(requiredConfidence, sampleStats.TotalCount, sampleStats.SuccessCount,
		...
		baseStats.FlakeCount, approvedRegression, numberOfIgnoredSampleJobRuns, baseRelease)
			...
			if cachedTestStatuses, ok := c.cachedFallbackTestStatuses.Releases[priorRelease]; ok {
				if cTestStats, ok := cachedTestStatuses.Tests[testIdentification]; ok {
					...
					priorTestStats := c.assessComponentStatus(requiredConfidence, sampleStats.TotalCount, sampleStats.SuccessCount,
						...
						cTestStats.FlakeCount, approvedRegression, numberOfIgnoredSampleJobRuns, priorRelease)
...
func (c *componentReportGenerator) generateComponentTestReport(baseStatus map[string]crtype.TestStatus,
	...
	for testIdentification, baseStats := range baseStatus {
		...
		if !ok {
			...
			baseRelease := c.BaseRelease.Release
			baseStats, baseRelease, testStats = c.matchBestBaseStats(testID, testIdentification, baseRelease, baseStats, sampleStats, requiredConfidence, approvedRegression, resolvedIssueCompensation)

			if baseRelease != c.BaseRelease.Release {
				log.Infof("Overrode base stats using release %s for Test: %s - %s", baseRelease, baseStats.TestName, testIdentification)
				overridenBaseMatches += 1
			}
	...
	log.Infof("BaseStats: %d, baseMatches: %d, baseMisses: %d.  Overridden base stats: %d", len(baseStatus), baseReleaseMatches, baseReleaseMisses, overridenBaseMatches)
...
func (c *componentReportGenerator) assessComponentStatus(requiredConfidence, sampleTotal, sampleSuccess, sampleFlake, baseTotal, baseSuccess, baseFlake int, approvedRegression *regressionallowances.IntentionalRegression, numberOfIgnoredSampleJobRuns int, baseRelease string) crtype.ReportTestStats {
	...
	testStats := crtype.ReportTestStats{
		...
		BaseStats: crtype.TestDetailsReleaseStats{
			Release: baseRelease,
			Start:   &c.BaseRelease.Start,
			End:     &c.BaseRelease.End,
			TestDetailsTestStats: crtype.TestDetailsTestStats{
