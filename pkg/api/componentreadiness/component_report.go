...
func (c *ComponentReportGenerator) generateComponentTestReport(basisStatusMap, sampleStatusMap map[string]crstatus.TestStatus) (crtype.ComponentReport, error) {
	...
	for testKeyStr := range keySet {
		cr := testdetails.TestComparison{Explanations: []string{}} // The actual stats we return over the API
		...
		if !sampleThere {
			...
			cr.ReportStatus = crtest.MissingSample
		} else {
			// Initialize the test analysis before we start passing it around to the middleware
			if basisThere {
				initTestAnalysisStruct(&cr, c.ReqOptions, sampleStatus, &basisStatus)
			} else {
				initTestAnalysisStruct(&cr, c.ReqOptions, sampleStatus, nil)
			}

			// Give middleware a chance to adjust parameters prior to analysis
			if err := c.middlewares.PreAnalysis(testKey, &cr); err != nil {
				return crtype.ComponentReport{}, err
			}

			c.assessComponentStatus(&cr, log.NewEntry(log.New()))
			if lastFailure := sampleStatus.LastFailure; !lastFailure.IsZero() {
				cr.LastFailure = &lastFailure // it's a copy, for pointer hygiene
			}
		}
		...
		updateCellStatus(
			rowIdentifications, columnIdentifications, testKey, cr, // inputs
