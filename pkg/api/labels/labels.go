...
type Applier struct {
	...
	// subtractInfraFailure is the applier's single function seam: the InfraFailure
	// summary subtraction. It is wired behind a function field so the per-label
	// side-effect dispatch (applySideEffects) can be unit tested with a closure
	// and no database, following the convention in
	// pkg/dataloader/infrafailurebackfill.Backfiller. The base label append itself
	// is a plain package function (appendProwJobRunLabel) covered by the
	// integration tests. NewApplier wires the real implementation.
