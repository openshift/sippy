...
func TestApplyValidatesRequestBeforeDatabase(t *testing.T) {
	request := ApplyRequest{
		RunID:        "123",
		ProwJobStart: time.Unix(1000, 0).UTC(),
		Release:      "4.18",
	}

	got, outcome := NewApplier(nil).Apply(context.Background(), request)

	if outcome != ApplyOutcomeError {
		t.Errorf("outcome = %d, want %d", outcome, ApplyOutcomeError)
	}
	if got.Error != "label is required" {
		t.Errorf("error = %q, want %q", got.Error, "label is required")
	}
}

func TestApplyRejectsUnavailableDatabase(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil receiver", applier: nilApplier},
		{name: "nil database wrapper", applier: NewApplier(nil)},
		{name: "nil GORM database", applier: NewApplier(&db.DB{})},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, outcome := tt.applier.Apply(context.Background(), request)

			if outcome != ApplyOutcomeError {
				t.Errorf("outcome = %d, want %d", outcome, ApplyOutcomeError)
			}
			if !strings.Contains(got.Error, "requires a database connection") {
				t.Errorf("error %q does not contain database unavailable detail", got.Error)
			}
			if got.RunID != "123" || got.Label != infrafailure.LabelInfraFailure {
				t.Errorf("result run_id/label = %q/%q, want %q/%q", got.RunID, got.Label, "123", infrafailure.LabelInfraFailure)
			}
		})
	}
}

func TestApplySideEffectsRejectsUnavailableInfraFailureSubtractor(t *testing.T) {
	var nilApplier *Applier
	tests := []struct {
		name    string
		applier *Applier
		...
		{name: "nil receiver", applier: nilApplier},
		{name: "nil subtractor", applier: &Applier{}},
	}
