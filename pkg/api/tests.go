...
func PrintTestsJSONFromDB(release string, w http.ResponseWriter, req *http.Request, dbc *db.DB) {
	...
	// Watchlist query param controls if we're going to automatically filter to only tests with the watchlist flag.
	watchlistStr := req.URL.Query().Get("watchlist")
	watchlist := false
	if watchlistStr == "true" {
		watchlist = true
	}
	...
	testsResult, overall, err := BuildTestsResults(dbc, release, period, collapse, watchlist, includeOverall, fil)
	if err != nil {
...
func PrintCanaryTestsFromDB(release string, w http.ResponseWriter, dbc *db.DB) {
	...
	results, _, err := BuildTestsResults(dbc, release, "default", true, false, false, &f)
	if err != nil {
...
func BuildTestsResults(dbc *db.DB, release, period string, collapse, watchlist, includeOverall bool, fil *filter.Filter) (testsAPIResult, *apitype.Test, error) { //lint:ignore
	now := time.Now()
	...
	if collapse {
		...
		variantSelect = "suite_name, variants, open_bugs, watchlist, " +
	...
	// should we filter down to only tests on the watchlist or not?
	if watchlist {
		log.Info("WATCHLIST REQUESTED")
		rawQuery = rawQuery.Where("watchlist = ?", true)
	}
