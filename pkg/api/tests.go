...
func PrintTestsJSONFromDB(release string, w http.ResponseWriter, req *http.Request, dbc *db.DB, cacheClient cache.Cache) {
	...
	testsResult, overall, err := buildTestsResultsFromPostgres(dbc, cacheClient, release, period, collapse, includeOverall, fil)
	if err != nil {
...
func buildTestsResultsFromPostgres(dbc *db.DB, cacheClient cache.Cache, release, period string, collapse, includeOverall bool, fil *filter.Filter) (testsAPIResult, *apitype.Test, error) { //lint:ignore
	matview := testReport7dMatView
	if period == "twoDay" {
		matview = testReport2dMatView
	}
	// TODO: wrap this with a redis cache lookup/set
	return buildTestsResults(dbc, release, matview, collapse, includeOverall, fil)
}

func buildTestsResults(dbc *db.DB, release, matview string, collapse, includeOverall bool, fil *filter.Filter) (testsAPIResult, *apitype.Test, error) { //lint:ignore
	now := time.Now()
	...
	if collapse {
		...
		rawQuery = query.TestsByNURPAndStandardDeviation(dbc, release, matview)
	...
	}).Debug("buildTestsResultsFromPostgres completed")
