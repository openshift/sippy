...
func (spec *TestResultsSpec) buildTestsResultsFromPostgres(ctx context.Context, dbc *db.DB, cacheClient cache.Cache) (testResults, error) {
	...
	generator := func(ctx context.Context) (testResults, []error) {
		return spec.buildTestsResults(ctx, dbc, matview)
	}
...
func (spec *TestResultsSpec) buildTestsResults(ctx context.Context, dbc *db.DB, matview string) (result testResults, errs []error) { //lint:ignore
	now := time.Now()
