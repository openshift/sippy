...
func BuildTestsResultsFromBigQuery(bqc *bq.Client, release, period string, collapse, includeOverall bool, fil *filter.Filter) (testsAPIResult, *apitype.Test, error) { //lint:ignore
	now := time.Now()
	...
	/*componentMappingString := fmt.Sprintf(`WITH latest_component_mapping AS (
		...
		WHERE created_at = (
			...
			FROM %s.component_mapping))`, dataSet, dataSet)
	variantString := fmt.Sprintf(`
	job_variants AS (
		SELECT
			job_name,
			ARRAY_AGG(CONCAT(variant_name, ':', variant_value)) AS variants
		FROM
			%s.job_variants
	...
	queryString := fmt.Sprintf(`WITH latest_component_mapping AS (
				...
				SELECT
					 junit.*,
					 cm.jira_component,
					 cm.jira_component_id,
					 jv.variants
				FROM '%s.%s' junit
				LEFT JOIN latest_component_mapping cm ON testsuite = cm.suite AND test_name = cm.name
				LEFT JOIN job_variants jv ON junit.job_name = jv.job_name
				WHERE release='%s'`,
	dataSet, dataSet, dataSet, dataSet, table, release)
	*/
	//rawQuery := dbc.DB.
	//	Table(table).
	//	Where("release = ?", release)
	...
	//if rawFilter != nil {
	//	queryStr += " AND " + rawFilter.ToBQStr(apitype.Test{})
	//}
	...
	if len(errs) > 0 {
		return []apitype.Test{}, nil, errs[0]
	}
...
func FetchTestResultsFromBQ(ctx context.Context, query *bigquery.Query) ([]apitype.Test, []error) {
	errs := []error{}
	result := []apitype.Test{}
