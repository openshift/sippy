...
func (spec *TestResultsSpec) buildTestsResultsFromPostgres(dbc *db.DB, cacheClient cache.Cache) (testResults, error) { //lint:ignore
	...
	generator := func(ctx context.Context) (testResults, []error) {
		return spec.buildTestsResults(dbc, matview)
	}
	result, errs := GetDataFromCacheOrMatview(context.TODO(), cacheClient,
...
func (spec *TestResultsSpec) buildTestsResults(dbc *db.DB, matview string) (result testResults, errs []error) { //lint:ignore
	now := time.Now()
