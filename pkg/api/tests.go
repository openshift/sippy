...
func (spec *TestResultsSpec) buildTestsResultsPGGenerator(ctx context.Context, dbc *db.DB, matview string) (result testResults, errs []error) {
	...
	if nameFilter != nil && len(nameFilter.Items) > 0 {
		testSubquery, err := nameFilter.ToSQL(dbc.DB.Model(&models.Test{}).Select("id"), apitype.Test{})
		if err != nil {
	...
	if !useCollapsedMatview && variantFilter != nil && len(variantFilter.Items) > 0 {
		rawQuery, err = variantFilter.ToSQL(rawQuery, apitype.Test{})
		if err != nil {
			return result, []error{err}
		}
	}
...
func (spec *TestResultsSpec) buildTestsResultsBQGenerator(ctx context.Context, bqc *bq.Client) (testResultsBQ, []error) {
	...
	if rawFilter != nil && len(rawFilter.Items) > 0 {
		filterResult, err := rawFilter.ToBQStr(apitype.Test{}, &paramIndex)
		if err != nil {
			return testResultsBQ{}, []error{err}
		}
	...
	`, query.QueryTestSummer, bqc.Dataset, table, bqc.Dataset, whereStr, query.QueryTestSummarizer)
		...
		if processedFilter != nil && len(processedFilter.Items) > 0 {
			filterResult, err := processedFilter.ToBQStr(apitype.Test{}, &paramIndex)
			if err != nil {
				return testResultsBQ{}, []error{err}
			}
			whereStr += " AND " + filterResult.SQL
