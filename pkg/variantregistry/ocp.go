...
func (v *OCPVariantLoader) setJobTierAndProcedure(_ logrus.FieldLogger, variants map[string]string, jobName string) {
	...
	for _, excluded := range excludedPatterns {
		if !strings.Contains(jobNameLower, excluded) {
			continue
		}
		variants[VariantJobTier] = "excluded"
		variants[VariantProcedure] = VariantNoValue
	...
	procedurePatterns := []struct {
		...
		{"-etcd-scaling", "etcd-scaling", "rare"},
		{"-cpu-partitioning", "cpu-partitioning", "rare"},
		{"-automated-release", "automated-release", "standard"},
	}

	for _, entry := range procedurePatterns {
		if strings.Contains(jobNameLower, entry.substring) {
			variants[VariantProcedure] = entry.procedure
			variants[VariantJobTier] = entry.jobTier
			return
		}
	}

	// Special case for "qe" owner - always excluded except jobs from above
	if variants[VariantOwner] == "qe" {
		variants[VariantJobTier] = "excluded" // Only include automated-release jobs for now
		variants[VariantProcedure] = VariantNoValue
		return
	}
