...
func (rs *ReleaseStream) baseReleaseStreamURL() string {
	return fmt.Sprintf("https://%s.%s/api/v1/releasestream", rs.Architecture, rs.Domain)
}

// buildReleaseStreams builds the full stream names for specified releases that match the project.
func buildReleaseStreams(releases map[string]v1.Release, architectures []string, project PayloadProject) []ReleaseStream {
	...
	product, domain := project.GetProductDomain()
