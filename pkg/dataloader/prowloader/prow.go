...
func (pl *ProwLoader) Load() {
	...
	errsCh := make(chan error, len(entries))
	total := len(entries)
	...
	for i := 0; i < pl.maxConcurrency; i++ {
		...
		go func(ctx context.Context) {
			...
			for pj := range queue {
				...
				if err := pl.prowJobToJobRun(ctx, pj); err != nil {
					errsCh <- err
					log.WithError(err).Warningf("couldn't import job %s/%s, continuing", pj.Spec.Job, pj.Status.BuildID)
				}
...
func (pl *ProwLoader) prowJobToJobRun(ctx context.Context, pj *prow.ProwJob) error {
	...
	gcsJobRun := gcs.NewGCSJobRun(bkt, path)
	junitMatches, err := gcsJobRun.FindAllMatches(ctx, "**junit*xml")
	if err != nil {
		return errors.Wrap(err, "error finding junit files")
	}
...
func (pl *ProwLoader) processGCSBucketJobRun(ctx context.Context, pj *prow.ProwJob, id uint64, path string, junitMatches []string, dbProwJob *models.ProwJob) error {
	rows, failures, overallResult, err := pl.prowJobRunTestsFromGCS(ctx, pj, uint(id), dbProwJob.ID, dbProwJob.Release, path, junitMatches)
	if err != nil {
		return err
	}
	...
	return pl.bulkInsertJobRunTests(ctx, rows)
}

func (pl *ProwLoader) bulkInsertJobRunTests(ctx context.Context, rows []prowJobRunTestRow) error {
	if len(rows) == 0 {
		return nil
	}
	...
	cleanup, err := db.CopyToTempTable(ctx, conn, "tmp_job_run_tests", rows,
		[]db.TempColumn[prowJobRunTestRow]{
	...
	if _, err := conn.Exec(ctx, `
		...
		SELECT DISTINCT test_name, NOW(), NOW()
		FROM tmp_job_run_tests
	...
	if _, err := conn.Exec(ctx, `
		...
		SELECT DISTINCT suite_name, NOW(), NOW()
		FROM tmp_job_run_tests
		WHERE suite_name != ''
		ON CONFLICT (name) DO NOTHING
	`); err != nil {
	...
	if _, err := conn.Exec(ctx, `
		WITH inserted AS (
			INSERT INTO prow_job_run_tests (
				prow_job_run_id, prow_job_id, prow_job_run_timestamp,
				prow_job_run_release, test_id, suite_id,
				status, duration, created_at, updated_at
			)
			SELECT
				tmp.prow_job_run_id, tmp.prow_job_id, tmp.prow_job_run_timestamp,
				tmp.prow_job_run_release, t.id, s.id,
				tmp.status, tmp.duration, NOW(), NOW()
			...
			RETURNING id, test_id, COALESCE(suite_id, 0) AS sid,
				prow_job_run_id, prow_job_run_timestamp, prow_job_run_release
		)
		INSERT INTO prow_job_run_test_outputs (
			prow_job_run_test_id, prow_job_run_test_timestamp,
			prow_job_run_test_release, output, created_at, updated_at
		)
		SELECT
			ins.id, ins.prow_job_run_timestamp, ins.prow_job_run_release,
			tmp.output, NOW(), NOW()
		FROM inserted ins
		JOIN tests t ON t.id = ins.test_id
		JOIN tmp_job_run_tests tmp
			ON tmp.test_name = t.name
			AND tmp.prow_job_run_id = ins.prow_job_run_id
		LEFT JOIN suites s2 ON s2.name = tmp.suite_name AND s2.deleted_at IS NULL
		WHERE tmp.output IS NOT NULL
			AND COALESCE(s2.id, 0) = ins.sid
	`); err != nil {
