...
func findBug(testName string) string {
	// https://search.svc.ci.openshift.org/?search=%s&maxAge=48h&context=2&type=bug
	...
	//fmt.Printf("search result from %s: %s\n", testName, string(body))
...
func printReport(testFailures map[string]TestFailureMeta) {
	...
	//fmt.Printf("Test Name,Sig,Jobs Failed,Job Failures Caused\n")
	...
	for _, meta := range failures {
		...
		//fmt.Printf("%s,%d,%s,%s\n", meta.name, meta.count, meta.sig, strings.Join(jobs, "|"))
