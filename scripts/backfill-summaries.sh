...
#
# Options:
#   --table TABLE     Table to backfill (daily-summaries, cumulative-summaries,
#                     variant-cumulative-summaries) [required]
...
if [[ -z "$TABLE" ]]; then
    ...
    echo "Valid tables: daily-summaries, daily-totals, cumulative-summaries, variant-cumulative-summaries" >&2
    exit 1
fi

case "$TABLE" in
    daily-summaries|daily-totals|cumulative-summaries|variant-cumulative-summaries) ;;
    *)
...
for ((offset=DAYS; offset>0; offset-=BATCH)); do
    ...
    oc -n "$NAMESPACE" run "$pod_name" --rm -i --restart=Never \
        ...
        }" 2>&1 | { grep -iE 'summar|error|elapsed|complete|panic|fatal|fail|timeout|warn' || true; } || true
