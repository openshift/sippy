  ...
  template:
    spec:
      ...
      - name: sippy
        ...
        command:  ["/bin/sh", "-c"]
        args:
          - /bin/sippy load prow --level=debug --release 4.7 --database-dsn=postgresql://postgres:password@postgres.sippy-e2e.svc.cluster.local:5432/postgres --mode=ocp --config ./config/openshift.yaml --google-service-account-credential-file /tmp/secrets/gcs-cred
        env:
